...
一个 Rust MCP 服务器，使用 Claude 子智能体对任何文档进行任何标准的评估，配备证据密度评分器（EDS）使幻觉在数学上可被检测。输入一份 PDF 和评估意图——返回结构化评分、弱点分析和完整审计轨迹。文档评估是核心差异化能力：BITF 与专家评分相差 2.8pp，而原始 Claude 偏差约 15pp。可选的预测可信度模块提供结构化提取与 SNN 验证。
...
子智能体预测与原始 Claude 表现一致（均为 5/5）。该模块的价值在于结构化提取（预测类型、时间框架、证据映射）+ SNN 验证 + 审计轨迹，而非优于基础模型的准确性。基于规则的回退方案在被证明有害后已被替换。
...
证据评分器使其可被检测。每个智能体的每项标准对应一个神经元。本体中的证据生成脉冲。无证据 = 无脉冲 = 不发放 = 零分。当 LLM 评 9/10 而证据评分器评 2/10 时，系统会标记：
