...
Rust製のMCPサーバーで、Claudeサブエージェントを使用してあらゆる文書をあらゆる基準で評価します。証拠密度スコアラー（EDS）により、幻覚を数学的に検出可能にします。PDFと評価意図を入力すると、構造化されたスコア、弱点分析、完全な監査証跡を返します。文書評価がコアの差別化要因：BITFは専門家スコアから2.8ppの偏差に対し、素のClaudeは約15pp。オプションの予測信頼性モジュールは証拠検証付きの構造化抽出を提供します。
...
**サブエージェントモード**（ELLIPSEコーパス、45編、1.0–5.0スケール）：
...
QWK 0.902は「信頼性のある」評価者間一致の0.80閾値を超えている。

**大規模決定論的モード**（証拠スコアラーのみ、LLMなし）：

| データセット | N | Pearson r | QWK | MAE |
| ---------- | - | --------- | --- | --- |
| ELLIPSE（層別45） | 45 | 0.442 | 0.258 | 1.08 |
| ELLIPSE（自然500） | 500 | 0.103 | 0.027 | 0.79 |
| ASAP Set 1（層別51） | 51 | 0.676 | 0.342 | 2.57 |
| ASAP Set 1（自然1000） | 1000 | 0.310 | 0.076 | 2.94 |

**決定論的スコアラーは大規模で失敗する。** 層別小サンプル（スコア帯に均等分布）は誤解を招く良い結果を示す。自然分布（80%以上のエッセイが2-3の隣接スコア帯に集中）では、スコアラーは区別できない。証拠スコアラーは**検証レイヤー**（ゼロ証拠幻覚の検出）であり、独立スコアラーではないことを確認。意味のある評価にはLLMサブエージェントが必要。
...
サブエージェント予測は素のClaudeと同等のパフォーマンス（両方5/5）。このモジュールの価値は構造化抽出（予測タイプ、タイムフレーム、証拠マッピング）+ 証拠検証 + 監査証跡にあり、ベースモデルを超える精度向上ではありません。ルールベースのフォールバックは有害と判明した後に置換されました。
...
MiroFishのエージェントは、裏付け証拠のない基準に9/10のスコアを「正当化」できる。これは信頼度付きの幻覚である。証拠スコアラーはこれを検出可能にする。

### 生物学的インスピレーション

スコアラーは[スパイキングニューラルネットワーク](https://en.wikipedia.org/wiki/Spiking_neural_network)（離散的な電気パルスで通信する第三世代ニューラルネットワーク）から4つの特性を借用している。これは神経形態コンピューティングではなく、生物学的に着想を得た動力学を使用する証拠密度スコアラーである。

### 特性 1：膜電位 + 閾値 = 最低証拠基準

各エージェントの各評価基準にニューロンが1つ。知識グラフからの証拠が入力スパイクを生成：

| 証拠タイプ | スパイク強度 | 例 |
| --------- | ---------- | -- |
| 定量データ | 0.8–1.0 | "FTSE 100が45%上昇" |
| 検証可能な主張 | 0.6–0.8 | "イングランド銀行が8950億ポンドの資産を購入" |
| 引用文献 | 0.5–0.7 | "(Bernanke, 2009)" |
| 一般的主張 | 0.3–0.5 | "QEは安定化ツールとして有効だった" |
| セクション整合 | 0.2–0.4 | セクションタイトルが基準と一致 |

スパイクは膜電位に蓄積。閾値を超えるとニューロンが発火。**証拠なし = スパイクなし = 発火なし = スコアゼロ。** これが反幻覚特性。
...
実際のニューロンは時間とともに電荷が漏れる。これを**収穫逓減**のモデルに使用 — 同じトピックについての10番目の引用は1番目より価値が低い。減衰なしでは、弱い証拠を50回繰り返してスコアを稼げてしまう。

### 特性 3：側方抑制 = 討論チャレンジ

```text
エージェントAがエージェントBのスコアにチャレンジした場合：
  エージェントBのニューロン.apply_inhibition(チャレンジ強度)
  → 膜電位が低下
  → 同じスコアを維持するにはより多くの証拠が必要
```

実際の神経ネットワークでは、隣接するニューロンが互いを抑制して応答を鋭くする。討論に利用：チャレンジされたスコアはより強い証拠がなければ生き残れない。

### 特性 4：不応期 = 二重計上の防止

発火後、ニューロンは不応期に入り新しいスパイクは無視される。同じ証拠が短時間で複数回計上されることを防ぐ。

### 実際のスコアリング式

生物学的フレーミングを剥がすと、数学は以下の通り：

```text
evidence_saturation = ln(1 + total_spikes) / ln(16)     // 対数スケール、~15項目で飽和
spike_quality       = mean(spike_strengths)               // 0.0–1.0
firing_rate         = fire_count / timesteps              // 伝統的SNNシグナル

raw_score = evidence_saturation × 0.50                    // 証拠の量
          + spike_quality       × 0.35                    // 証拠の強さ
          + firing_rate         × 0.15                    // 蓄積の一貫性

final = raw_score × (1.0 - inhibition) × max_score       // 討論でチャレンジされたら減点
```

**平たく言えば：** スコア = 50%「証拠量」+ 35%「証拠の強さ」+ 15%「蓄積の一貫性」。他のエージェントからのチャレンジでペナルティ。

### なぜ単純に証拠を数えないのか？

加重合計で80%は達成できる。SNN着想の特性が追加する4つの能力：

1. **時間的動態** — 証拠が1セクションに集中（バースト）vs 文書全体に分散で、異なる発火パターンが生まれる
2. **討論からの抑制** — 単純なカウンターでは「このスコアはチャレンジされた」をモデル化できない
3. **不応期** — 同タイプの証拠によるスコアの水増しを防止（同じ著者の5回の引用がそれぞれフルクレジットを得ない）
4. **閾値ベースの発火** — 自然な最低証拠基準を作成、任意の最低スコアよりエレガント

### 幻覚検出

LLMと証拠スコアラーが不一致の場合、システムがフラグを立てる：
...
最終スコアは両方をブレンド：`最終 = eds × eds重み + llm × llm重み`。スコアラーは決定論的：同じ証拠は常に同じスコアを生成。証拠が豊富ならスコアラーが支配。少なければLLMが補完 — ただし幻覚フラグが発動。
